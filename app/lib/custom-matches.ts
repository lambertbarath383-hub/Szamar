...
const CUSTOM_MATCHES_API_ENDPOINT = "/api/custom-matches";
...
export function writeCustomMatchEntriesToStorage(entries: CustomMatchEntry[]) {
  ...
  void pushCustomMatchEntriesToServer(entries);
}

export async function pushCustomMatchEntriesToServer(entries: CustomMatchEntry[]) {
  await fetch(CUSTOM_MATCHES_API_ENDPOINT, {
    method: "PUT",
    headers: {
...
export async function syncCustomMatchEntriesFromServer() {
  if (typeof window === "undefined") {
    return [];
  }
  const response = await fetch(CUSTOM_MATCHES_API_ENDPOINT, { cache: "no-store" });
  const payload = (await response.json()) as { ok?: boolean; data?: CustomMatchEntry[] };
  if (!response.ok || !payload.ok || !Array.isArray(payload.data)) {
    throw new Error("Nem sikerült szinkronizálni a meccseket.");
  }
  window.localStorage.setItem(CUSTOM_MATCHES_STORAGE_KEY, JSON.stringify(payload.data));
  window.dispatchEvent(new Event(CUSTOM_MATCHES_CHANGED_EVENT));
  return payload.data;
}
